...
// Helper function to process and normalize attachment data
export const processAttachment = (attachment: any): any => {
  if (!attachment) return attachment;

  // If attachment is already properly structured, return as-is
  if (attachment.data && attachment.file_name && attachment.content_type) {
    return attachment;
  }

  // If it's a string, assume it's file data that needs processing
  if (typeof attachment === 'string') {
    ...
    if (isBase64(attachment)) {
      // It's Base64 but missing other fields
      return {
        data: attachment,
      ...
      // Convert to Base64
      try {
        const buffer = Buffer.from(attachment, 'binary');
        return {
        ...
        // Fallback
        const buffer = Buffer.from(attachment, 'utf8');
        return {
          data: buffer.toString('base64'),
          file_name: 'attachment',
  ...
  // If it's an object but missing required fields, try to normalize
  if (typeof attachment === 'object') {
    const processed = { ...attachment };
    
    // Ensure data field exists and is Base64
    if (processed.file_data && !processed.data) {
      processed.data = processed.file_data;
      delete processed.file_data;
    }
    
    if (processed.data && !isValidBase64(processed.data)) {
      try {
        const buffer = Buffer.from(processed.data, 'binary');
        processed.data = buffer.toString('base64');
      } catch {
        const buffer = Buffer.from(processed.data, 'utf8');
        processed.data = buffer.toString('base64');
      }
    }
    
    // Set default values for missing fields
    if (!processed.file_name) processed.file_name = 'attachment';
    if (!processed.content_type) processed.content_type = 'application/octet-stream';
